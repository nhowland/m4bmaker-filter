...
class TestEstimateStorageBytes:
    ...
    def test_matches_the_real_13_5_hour_fixture_measurement(self) -> None:
        ...
        # per intermediate stage" — the real test audiobook fixture,
...
class TestDefaultOutputPath:
    ...
    def test_appends_filtered_suffix_before_extension(self) -> None:
        with patch("m4bmaker.filter.renderer.output_dir_override", return_value=None):
            result = default_output_path(Path("/books/Test Audiobook.m4b"))
        assert result == Path("/books/Test Audiobook (filtered).m4b")
